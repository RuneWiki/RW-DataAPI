...
export default class Js5Index {
    ...
    groupDigests: BigInt64Array | null = null;
    ...
    decode(bytes: Int8Array): void {
        const buf: JagBuffer = new JagBuffer(bytes);
        ...
        for (let i: number = 0; i < this.size; i++) {
            if (protocol >= 7) {
                this.groupIds[i] = prevGroupId += buf.g2or4();
            } else {
        ...
        this.groupDigests = new BigInt64Array(this.capacity);
        ...
        if (hasDigests) {
            for (let i: number = 0; i < this.size; i++) {
                this.groupDigests[this.groupIds[i]] = buf.g8();
            }
        }
