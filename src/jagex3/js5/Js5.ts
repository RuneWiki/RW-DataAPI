...
export default class Js5 {
    ...
    private openrs2: OpenRS2;

    constructor(index: Js5Index, archive: number, openrs2: OpenRS2) {
        this.index = index;
    ...
    unpackGroup(group: number, key: number[] | null = null): boolean {
        ...
        let compressed: Int8Array = new Int8Array();
        ...
        if (groupSize > 1) {
            ...
            const buf: JagBuffer = new JagBuffer(uncompressed);
            buf.pos = position;
            ...
            const extracted: Int8Array[] = new Array(groupSize);
            for (let i: number = 0; i < groupSize; i++) {
                extracted[i] = new Int8Array(lens[i]);
                lens[i] = 0;
            }
